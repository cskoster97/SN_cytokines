...
# Filter metadata to include only rows with treatment identifier "TGF"
...
write.csv(metadata, "Metadata_filtered_TGFb-noSN20.csv")
...
write.csv(counts, "Rawcounts_filtered_TGFb-noSN20.csv")
...
write.xlsx(as.data.frame(log2_normalized_counts), "log2normalized_counts_TGFb-noSN20.xlsx", rowNames = TRUE)
```
...
#Save results table
write.csv(sub_res_sig, file = "./Results TGFb-noSN20/LogFoldChanges_full_TGFb-noSN20.csv", row.names = TRUE)
```

Extract the normalized counts
```{r}
normalized_counts <- counts(ddsoverall, normalized = TRUE) #extracted counts normalized by estimateSIzeFactors()
write.csv(normalized_counts, file.path("./Results TGFb-noSN20/Normalized Counts TGFb-noSN20.csv"))
```
...
EnhancedVolcano(sub_res_sig,
                ...
                xlim = c(-5, 7), #edit
                ylim = c(0, 25), #edit
                title = "Effect of TGFb on hPSC-derived sensory neurons",
                pCutoff = 0.05,
                FCcutoff = 1.5, #1
                pointSize = 2.0,
                labSize = 4.0,
