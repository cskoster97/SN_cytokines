...
setwd("G:/My Drive/RNAseq/Carli_SN+Cytokines/Analysis Carli/Results TGFb/")
...
```{r}
sig_DEGs <- read.csv("LogFoldChanges_full_TGFb.csv", sep = ",", row.names = 1)
...
saveWorkbook(wb, "H-pathways-TGFb.xlsx") # Save the workbook to a file
...
addWorksheet(wb, "DataTable") # Add a worksheet to the workbook
writeDataTable(wb, "DataTable", datatable_data) # Write the data from the DT::datatable to the Excel worksheet
saveWorkbook(wb, "C2CP-pathways-TGFb.xlsx") # Save the workbook to a file
...
addWorksheet(wb, "DataTable") # Add a worksheet to the workbook
writeDataTable(wb, "DataTable", datatable_data) # Write the data from the DT::datatable to the Excel worksheet
saveWorkbook(wb, "C5BP-pathways-TGFb.xlsx") # Save the workbook to a file
...
datatable_data <- as.data.frame(tidy_fgsea_c7IM) # Extract the data from the DT::datatable
wb <- createWorkbook() # Create a new Excel workbook
addWorksheet(wb, "DataTable") # Add a worksheet to the workbook
writeDataTable(wb, "DataTable", datatable_data) # Write the data from the DT::datatable to the Excel worksheet
saveWorkbook(wb, "C7IM-pathways-TGFb.xlsx") # Save the workbook to a file

collapsed_c7IM <- collapsePathways(fgsea_c7IM[order(pval)][padj < 0.05],
                                         c7IM_pathways,
                                         ranks)
main_c7IM <- fgsea_c7IM[pathway %in% collapsed_c7IM$mainPathways][order(-NES), pathway]
main_res_c7IM <- fgsea_c7IM[fgsea_c7IM$pathway %in% main_c7IM] 
main_res_c7IM$pathway <- sub("GOBP_","",main_res_c7IM$pathway)

num_top_pathways <- 50 # Choose the number of top pathways to display

# Select the top pathways based on NES
top_main_res_c7IM <- main_res_c7IM %>%
  top_n(num_top_pathways, wt = NES)

ggplot(top_main_res_c7IM, aes(reorder(pathway, NES), NES)) +
  geom_col(fill='dodgerblue') +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="GSEA of Immunologic Signiatures in Sensory Neurons exposed to TGFb") + 
  theme_minimal() +
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5, vjust = 0.5),  # Center the title
    axis.text.y = element_text(size = 12)  # Adjust axis text font size
